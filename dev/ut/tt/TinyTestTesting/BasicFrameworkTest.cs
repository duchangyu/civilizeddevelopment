...
{
    /// <summary>
    /// Unit test class that verifies test classes are loaded
    /// and test methods are executed. If this class is not displayed
    /// in the output, it means the framework is not working correctly.
    /// </summary>
    [TestClass]
    public class BasicFrameworkTesting
    {
        public BasicFrameworkTesting()
        {
            m_SetupExecutedCount = 0;
            m_SetupExecutedExpectedCount = 0;
            m_TeardownExecutedCount = 0;
            m_TeardownExecutedExpectedCount = 0;
        }

        public void Setup()
        {
            m_SetupExecutedCount++;
        }

        public void Teardown()
        {
            m_TeardownExecutedCount++;
        }

        [TestMethod]
        public void MethodIsExecutedByFramework()
        {
            startTest();
            Assert.IsTrue(true);
            endTest();
        ...
        [TestMethod]
        public void SetupIsExecutedIfImplemented()
        {
            startTest();
            Assert.AreEqual(m_SetupExecutedExpectedCount, m_SetupExecutedCount, "Setup should have been executed.");
            endTest();
        }

        [TestMethod]
        public void TeardownIsExecutedIfImplemented()
        {
            startTest();
            Assert.AreEqual(m_TeardownExecutedExpectedCount, m_TeardownExecutedCount, "Teardown should have been executed.");
            endTest();
        }

        private void startTest()
        {
            m_SetupExecutedExpectedCount++;
        }

        private void endTest()
        {
            m_TeardownExecutedExpectedCount++;
        }

        private int m_SetupExecutedCount;
        private int m_SetupExecutedExpectedCount;
        private int m_TeardownExecutedCount;
        private int m_TeardownExecutedExpectedCount;
    }
}