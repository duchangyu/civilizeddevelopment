...
{
    /// <summary>
    /// Allows reading a test run configuration from an XML file.
    /// </summary>
    public class XMLConfiguration : ITestRunConfiguration
    {
        /// <summary>
        /// Class constructor that initializes the object by reading the
        /// specified XML file.
        /// </summary>
        /// <param name="configurationFile">Name of the XML configuration file.</param>
        public XMLConfiguration(string configurationFile)
        {
            m_TestModules = new List<string>();
            ...
            while (reader.Read())
            {
                switch(reader.NodeType)
                {
                    case XmlNodeType.Element:
                        if (reader.Name == "test_module")
                        {
                            m_TestModules.Add(reader.ReadElementContentAsString());
                        }
                        break;
                }
            }
        ...
        /// <summary>
        /// Enumerates the modules (DLLs) that were spcified in the configuration.
        /// </summary>
        public IEnumerable<string> TestModules
        {
            get
            {
                return m_TestModules;
            }
        }

        private List<string> m_TestModules;
    }
}