...
{
    /// <summary>
    /// Contains information about a test module (DLL).
    /// </summary>
    internal class TestModuleInfo
    {
        /// <summary>
        /// Class constructor that initializes the object from the module (DLL)
        /// file name.
        /// </summary>
        /// <param name="moduleName">File name of the test module.</param>
        internal TestModuleInfo(string moduleName)
        {
            Assembly assm = Assembly.LoadFrom(moduleName);
            m_TestClasses = loadTestClassesFromAssembly(assm);
        }

        /// <summary>
        /// Enumerates information objects about the test classes in the module.
        /// </summary>
        internal IEnumerable<TestClassInfo> TestClasses 
        {
            get
            {
        ...
        private List<TestClassInfo> loadTestClassesFromAssembly(Assembly assm)
        {
            List<TestClassInfo> testClassesFound = new List<TestClassInfo>();
            foreach(Type type in assm.GetTypes())
            {
                if (type.GetCustomAttributes(typeof(TestClassAttribute), false).Length > 0)
                {
                    testClassesFound.Add(new TestClassInfo(type));
                }
            }
            return testClassesFound;
        }

        private List<TestClassInfo> m_TestClasses;
    }
}